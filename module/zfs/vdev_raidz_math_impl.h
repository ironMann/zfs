 ...
 */
static int
raidz_zero_abd(void *dc, uint64_t dsize, void *private)
{
	v_t *dst = (v_t *) dc;
	size_t i;
...
{									\
	abd_iterate_wfunc(dabd, size, raidz_zero_abd, NULL);		\
}
 ...
 */
static int
raidz_copy_abd(void *dc, void *sc, uint64_t dsize, uint64_t ssize,
    void *private)
{
	v_t *dst = (v_t *) dc;
	...
	for (i = 0; i < ssize / sizeof (v_t); i += (2 * COPY_STRIDE)) {
		LOAD(src + i, COPY_D);
		STORE(dst + i, COPY_D);
...
{									\
	abd_iterate_func2(dabd, sabd, size, size, raidz_copy_abd, NULL);\
}									\
...
raidz_add_abd(void *dc, void *sc, uint64_t dsize, uint64_t ssize, void *private)
{
	...
	for (i = 0; i < ssize / sizeof (v_t); i += (2 * ADD_STRIDE)) {
		LOAD(dst + i, ADD_D);
		XOR_ACC(src + i, ADD_D);
		STORE(dst + i, ADD_D);
...
raidz_generate_p_impl(raidz_map_t * const rm)
{
	...
	raidz_copy(pabd, rm->rm_col[1].rc_data, psize);
	...
	for (c = 2; c < ncols; c++) {
		...
		/* add data column */
		abd_iterate_func2(pabd, dabd, dsize, dsize, raidz_add_abd,
		    NULL);
	}
...
raidz_generate_pq_impl(raidz_map_t * const rm)
{
	...
	raidz_copy(cabds[CODE_P], rm->rm_col[2].rc_data, csize);
	raidz_copy(cabds[CODE_Q], rm->rm_col[2].rc_data, csize);
...
raidz_generate_pqr_impl(raidz_map_t * const rm)
{
	...
	raidz_copy(cabds[CODE_P], rm->rm_col[3].rc_data, csize);
	raidz_copy(cabds[CODE_Q], rm->rm_col[3].rc_data, csize);
	raidz_copy(cabds[CODE_R], rm->rm_col[3].rc_data, csize);
...
raidz_reconstruct_p_impl(raidz_map_t *rm, const int *tgtidx)
{
	...
	/* copy P into target */
	raidz_copy(xabd, rm->rm_col[CODE_P].rc_data, xsize);
...
raidz_reconstruct_q_impl(raidz_map_t *rm, const int *tgtidx)
{
	...
	if (firstdc != x) {
		raidz_copy(xabd, rm->rm_col[firstdc].rc_data, xsize);
	} else {
...
raidz_reconstruct_pq_impl(raidz_map_t *rm, const int *tgtidx)
{
	...
	if (firstdc != x) {
		raidz_copy(xabd, rm->rm_col[firstdc].rc_data, xsize);
		raidz_copy(yabd, rm->rm_col[firstdc].rc_data, xsize);
	} else {
		raidz_zero(xabd, xsize);
	...
	if (ysize < xsize)
		raidz_copy(rm->rm_col[y].rc_data, yabd, ysize);
...
raidz_reconstruct_pqr_impl(raidz_map_t *rm, const int *tgtidx)
{
	...
	if (firstdc != x) {
		raidz_copy(xabd, rm->rm_col[firstdc].rc_data, xsize);
		raidz_copy(yabd, rm->rm_col[firstdc].rc_data, xsize);
		raidz_copy(zabd, rm->rm_col[firstdc].rc_data, xsize);
	} else {
		raidz_zero(xabd, xsize);
		raidz_zero(yabd, xsize);
	...
	if (ysize < xsize)
		raidz_copy(rm->rm_col[y].rc_data, yabd, ysize);
	if (zsize < xsize)
		raidz_copy(rm->rm_col[z].rc_data, zabd, zsize);
