 ...
 */
static int
raidz_zero_abd(void *dc, size_t dsize, void *private)
{
	v_t *dst = (v_t *) dc;
	size_t i;
...
{									\
	abd_iterate_func(dabd, 0, size, raidz_zero_abd, NULL);		\
}
 ...
 */
static int
raidz_copy_abd(void *dc, void *sc, size_t size, void *private)
{
	v_t *dst = (v_t *) dc;
...
{									\
	abd_iterate_func2(dabd, sabd, 0, 0, size, raidz_copy_abd, NULL);\
}									\
 ...
 */
static int
raidz_add_abd(void *dc, void *sc, size_t size, void *private)
{
	v_t *dst = (v_t *) dc;
	const v_t *src = (v_t *) sc;
	size_t i;
...
raidz_generate_p_impl(raidz_map_t * const rm)
{
	...
	for (c = 2; c < ncols; c++) {
		...
		/* add data column */
		abd_iterate_func2(pabd, dabd, 0, 0, size, raidz_add_abd,
		    NULL);
	}
...
raidz_reconstruct_p_impl(raidz_map_t *rm, const int *tgtidx)
{
	...
	for (c = firstdc; c < ncols; c++) {
		...
		abd_iterate_func2(xabd, dabd, 0, 0, size, raidz_add_abd, NULL);
	}
...
raidz_reconstruct_q_impl(raidz_map_t *rm, const int *tgtidx)
{
	...
	abd_iterate_func2(xabd, rm->rm_col[CODE_Q].rc_abd, 0, 0, xsize,
		raidz_add_abd, NULL);
...
raidz_reconstruct_r_impl(raidz_map_t *rm, const int *tgtidx)
{
	...
	abd_iterate_func2(xabd, rm->rm_col[CODE_R].rc_abd, 0, 0, xsize,
		raidz_add_abd, NULL);
