...
const uint8_t sse_gf_mod_lt[2*256][16];
const uint8_t sse_clmul_mod_lt[2*256][16];
...
{									\
	...
	case 4:								\
		__asm(							\
		    "prefetchnta 16*" #offset "+0x00(%[MEM])\n"		\
		    "prefetchnta 16*" #offset "+0x20(%[MEM])\n"		\
		    : : [MEM] "r" (ptr));				\
		break;							\
	default:							\
		__asm(							\
...
{									\
	__asm(								\
	    ...
	    "movd %[mask], %%" _0f "\n"					\
	    "pshufd $0x0, %%" _0f ", %%" _0f "\n"			\
	    "movdqa 0x00(%[mod]), %%" _lt_mod_a "\n"			\
	    "movdqa 0x00(%[clmul]), %%" _lt_clmul_a "\n"		\
	    /* upper part */						\
	    ...
	    /* lts for lower part */					\
	    "movdqa 0x10(%[mod]), %%" _lt_mod_a "\n"			\
	    "movdqa 0x10(%[clmul]), %%" _lt_clmul_a "\n"		\
	    "movdqa %%" _lt_mod_a ", %%" _lt_mod_b "\n"			\
	    "movdqa %%" _lt_clmul_a ", %%" _lt_clmul_b "\n"		\
	    /* lower part */						\
