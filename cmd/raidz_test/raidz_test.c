...
cmp_code(raidz_test_opts_t *opts, const raidz_map_t *rm, const int parity)
{
	...
	for (i = 0; i < parity; i++) {
		if (0 != abd_cmp(CODE_COL(rm, i), CODE_COL(opts->rm_golden, i),
			CODE_COL_SIZE(rm, i))) {
			ret++;
...
cmp_data(raidz_test_opts_t *opts, raidz_map_t *rm)
{
	...
	for (i = 0; i < dcols; i++) {
		if (0 != abd_cmp(DATA_COL(opts->rm_golden, i), DATA_COL(rm, i),
...
static int
corrupt_rand(void *data, uint64_t size, void *private)
{
	int i;
	int *dst = (int *) data;
...
corrupt_colums(raidz_map_t *rm, const int *tgts, const int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		abd_iterate_wfunc(col->rc_data, col->rc_size, corrupt_rand,
		    NULL);
	}
}
