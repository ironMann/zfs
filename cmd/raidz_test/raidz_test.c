...
static void usage(boolean_t requested)
{
	...
	"\t[-h (print help)]\n"
	"\t[-D poor man's debugger (attach gdb on SIGSEGV)]\n"
	"",
	o->rto_ashift,						/* -a */
...
corrupt_raidz_map_data_colum(raidz_map_t *rm, int col)
{
	...
	abd_iterate_wfunc(DATA_COL(rm, col), DATA_COL_SIZE(rm, col),
		init_rand, NULL);
}
...
init_zio_data(zio_t *zio)
{
	...
	abd_iterate_wfunc(zio->io_data, zio->io_size, init_rand, NULL);
}
...
init_raidz_maps(void)
{
	...
	const size_t total_ncols = rto_opts.rto_dcols + PARITY_PQR;
	...
	err |= compare_raidz_maps_code(rm_test, rm_golden, PARITY_PQR);
...
fini_raidz_maps(void)
{
	...
	abd_free(zio_golden.io_data, zio_golden.io_size);
	bzero(&zio_golden, sizeof (zio_t));

	/* tear down test zio */
	abd_free(zio_test.io_data, zio_test.io_size);
...
run_rec_check(void)
{
	...
	/* create suitable raidz_map */
	rm_test = vdev_raidz_map_alloc(&zio_test, rto_opts.rto_ashift,
		rto_opts.rto_dcols + PARITY_PQR, PARITY_PQR);
	/* generate parity */
	vdev_raidz_generate_parity(rm_test);
...
main(int argc, char **argv)
{
	...
	if (sigaction(SIGUSR1, &action, NULL) < 0) {
		(void) fprintf(stderr, "ztest: cannot catch SIGUSR1: %s.\n",
		    strerror(errno));
		exit(EXIT_FAILURE);
	}
